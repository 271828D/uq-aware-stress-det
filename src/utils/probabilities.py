...
def get_prediction_probabilities(model: Any, X: Any) -> np.ndarray:
    """
    Get prediction probabilities from a model, handling both probabilistic and non-probabilistic models.
    ...
    if hasattr(model, "predict_proba"):
        # If yes, get the probabilities and return the column for the "stress" class (index 1)
        logger.info("✅ Model supports predict_proba. Using direct probabilities.")
        return model.predict_proba(X)[:, 1]
    
    else:
        # If the model cannot give probabilities, we need to convert its raw scores
        logger.info("⚠️ Model lacks predict_proba. Converting decision_function to probabilities.")
        
        # Get the raw decision scores from the model (distance from the decision boundary)
