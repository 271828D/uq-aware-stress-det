...
def calculate_all_metrics(y_true: pd.Series, y_pred: Any, y_prob: Any) -> Dict[str, float]:
    """
    ...
    """
    # First, break down the predictions into True Positives, False Positives, etc.
    ...
    metrics = {
        ...
        # Precision: Of all the times we predicted "stress", how often were we right?
        "precision": precision_score(y_true, y_pred, zero_division=0),
        
        # Recall (Sensitivity): Of all the actual "stress" cases, how many did we catch?
        "recall": recall_score(y_true, y_pred, zero_division=0),
        
        # F1-Score: A balanced average of Precision and Recall
        "f1_score": f1_score(y_true, y_pred, zero_division=0),
        
        # ROC-AUC: How well does the model separate stressed from non-stressed overall?
        "roc_auc": roc_auc_score(y_true, y_prob),
        
        # Specificity: Of all the "not stressed" people, how many did we correctly identify?
        ...
        # Matthews Correlation Coefficient: A balanced measure even if classes are very imbalanced
        "mcc": matthews_corrcoef(y_true, y_pred),
    }
    
    # Log the most important metric (F1-score) for quick feedback
    logger.info(f"📊 Calculated 9 metrics. F1-Score: {metrics['f1_score']:.4f}")
